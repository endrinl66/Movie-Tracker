...
function ShowDetail() {
  ...
  useEffect(() => {
    const loadShow = async () => {
      ...
      setError(null)
      try {
        const ingestRes = await api.post(`/shows/ingest/${tmdbId}`)
        const detailRes = await api.get(`/shows/${ingestRes.data.id}`)
        setShow(detailRes.data)
      } catch (err) {
  console.error('Failed to load show:', err)
  setError(t('detail.couldNotLoadShow'))
} finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    if (!show || !show.overview) return
    if (i18n.language === 'en') {
      setDisplayedOverview(show.overview)
      return
    }
    let cancelled = false
    translateText(show.overview, i18n.language)
      .then((translated) => {
        if (!cancelled) setDisplayedOverview(translated)
      })
      .catch(() => {
        if (!cancelled) setDisplayedOverview(show.overview)
      })
  ...
  const handleSetWatchStatus = async (status) => {
    ...
    try {
      await api.post('/watch-status', { show_id: show.id, status })
      ...
      <div className="flex flex-col md:flex-row gap-6 md:gap-10">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-bold mb-2">
            {show.title} {show.first_air_year && <span className="text-slate-400 font-normal">({show.first_air_year})</span>}
          </h1>
          ...
          <p className="text-slate-400 text-sm mb-2">
            {show.number_of_seasons} {t('detail.seasons')} · {show.number_of_episodes} {t('detail.episodes')}
          </p>

          <p className="text-slate-300 mb-6">{displayedOverview ?? show.overview}</p>

          {show.cast.length > 0 && (
            <div className="mb-8">
              <h2 className="text-lg font-semibold mb-2">{t('detail.cast')}</h2>
              <div className="flex flex-wrap gap-2">
                {show.cast.slice(0, 8).map((c) => (
      ...
      <div className="mt-10 border-t border-slate-800 pt-8">
        {user ? (
          <form onSubmit={handleRate} className="max-w-md">
            <h2 className="text-lg font-semibold mb-4">{t('detail.rateShow')}</h2>
          ...
          <p className="text-slate-400">
            <a href="/login" className="text-blue-400 hover:underline">{t('nav.login')}</a> {t('detail.loginToRateShow')}
          </p>
        )}
      </div>
